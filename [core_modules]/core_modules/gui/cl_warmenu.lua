...
local function drawButton(text, subText)
	...
	if multiplier then
		...
		if subText then
			drawText(subText, menus[currentMenu].x + buttonTextXOffset + menuWidth - 0.02, y - buttonHeight / 2 + buttonTextYOffset, buttonFont, subTextColor, buttonScale, false, shadow, true)
		end
	end
end
...
local function drawArrows()
    ...
    if HasStreamedTextureDictLoaded(-460811553) then
        local colour = menus[currentMenu].subTitleBackgroundColor

		if HasStreamedTextureDictLoaded("menu_textures") and HasStreamedTextureDictLoaded("character_creator_menu_textures") then
			SetScriptGfxDrawOrder(0)
			DrawSprite("menu_textures", "translate_bg_1a", x, y, menus[currentMenu].width + 0.02, buttonHeight, 0.0, 0, 0, 0, 255, 0)
			DrawSprite("character_creator_menu_textures", "character_creator_heritage", x, y, 0.0800, 0.08, 0.0, 0, 255, 255, 255, 0)
		end
...
function WarMenu.CreateMenu(id, title)
	...
	menus[id].subTitle = '         Options'
	menus[id].desTitle = 'Customized your characters appearance'
...
function WarMenu.ComboBox(text, items, currentIndex, selectedIndex, callback)
	...
	local selectedItem = tostring(currentIndex)
	local isCurrent = menus[currentMenu].currentOption == (optionCount + 1)

	-- if itemsCount > 1 and isCurrent then
	-- 	selectedItem = '← '..tostring(currentIndex)..' →'
	-- end
	...
	if WarMenu.Button(text, "←→", descText) then
	...
	elseif isCurrent then
		if not advanced then
			...
			if currentKey == keys.left then
				if currentIndex > -1.0 then if currentIndex == 0.05 then currentIndex = 0.0 else currentIndex = ( currentIndex - 0.05 ) end else currentIndex = 1.0 end
				selectedIndex = currentIndex
				callback(currentIndex, selectedIndex)
				return true
			elseif currentKey == keys.right then
