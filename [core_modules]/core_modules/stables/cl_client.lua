...
function SetHorseNameOnBuy(i)
	Citizen.CreateThread( function()
		AddTextEntry("FMMC_MPM_TYP8", "Name of horse:")
		...
		if (GetOnscreenKeyboardResult()) then
			...
			TriggerEvent('z00thorses:spawnHorse', Config.Horses[i]['Param'], "Theodore", 0x106961A8, 0x127E0412, 0x130E341A, 0x12DBBBAF, 0x1D4EDB88, 0x12F0DF9F, 0x3B3AB08)
		end
	end)
end
			...
			elseif WarMenu.Button(Config.Language["optionTameHorse"],'',Config.Language["optionTameHorseDesc"]) then
				...
				if isMounted then
					if not isOwned then
						...
						exports["rp-notify"]:DisplayLeftNotification("Stables", Config.Language["notifHasOwner"], "HUD_TOASTS", "toast_player_deadeye", 10000)
					end
				else
					exports["rp-notify"]:DisplayLeftNotification("Stables", Config.Language["notifNeedBeMount"], "HUD_TOASTS", "toast_player_deadeye", 10000)
				end
...
AddEventHandler('z00thorses:spawnHorse', function(horseData, horseName, id, saddle, blanket, mane, tail, bag, bedroll, stirups)
  myHorse[1] = tonumber(horseData["Model"])
  myHorse[2] = id
  myHorse[3] = horseName
  ...
  if myHorse[1] ~= 0 then
	...
	Citizen.CreateThread(function()
        ...
        while not HasModelLoaded(myHorse[1]) do
			...
			Citizen.InvokeNative(0x931B241409216C1F, PlayerPedId(), myHorse[4] , 1)
